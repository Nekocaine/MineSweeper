...
public class Plate extends Object{
	 ...
	 * @param y
	 */
	public Plate(int length, int height, int nbBombs, int x, int y) {
		this.length = length;
		this.height = height;
		this.nbBombs = nbBombs;
		this.world = new Case[height][length];
		for (int i = 0; i < length; i++) {
			for (int j = 0; j < height; j++) {
				world[j][i] = new Case();
			}
		}
		this.nbHiddenCases = height * length;
		int xr,yr;
		for (int i = 0; i < nbBombs; i++) {
			do {
				xr = (int)(Math.random()*(length));
				yr = (int)(Math.random()*(height));
		...
		for (int i = 0; i < length; i++) {
			for (int j = 0; j < height; j++) {
				...
				for (int k = (i-1 < 0)? 0 : i-1; k < ((i+2 > length)? length : i+2); k++) {
					for (int l = (j-1 < 0)? 0 : j-1; l < ((j+2 > height)? height : j+2); l++) {
	...
	private void decouvrir(int x, int y){
		...
		if(world[y][x].getNbBombsNear() == 0){
			for (int i = (x-1 < 0)? 0 : x-1; i < ((x+2 > length)? length : x+2); i++) {
				for (int j = (y-1 < 0)? 0 : y-1; j < ((y+2 > height)? height : y+2); j++) {
	...
	public void setIni(int length, int height, int nbBombs, int x, int y){
		this.length = length;
		this.height = height;
		this.nbBombs = nbBombs;
		this.world = new Case[height][length];
